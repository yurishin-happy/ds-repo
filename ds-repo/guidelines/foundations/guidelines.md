...
- **Surface strategy**: **Layered structure**
    ...
    `surface-primary` is the default — start here. Most of what users see lives on this surface.

    | Role | Token | When it appears |
    |------|-------------|------|
    | Floor | `surface-secondary` | Page background — only visible as gutters and gaps between content sections |
    | Canvas *(default)* | `surface-primary` | Main content surface — the dominant visible layer (articles, sections, page panels) |
    | Raised | `surface-tertiary` | Small elements elevated on the canvas — chips, score boxes, inline containers |
